...
rl.question('Neues Admin-Passwort (mindestens 14 Zeichen, Eingabe verborgen): ',async password=>{mute=false;rl.close();if(password.length<14){console.error('\nPasswort zu kurz.');process.exitCode=1;return;}console.log('\nADMIN_PASSWORD_HASH='+await hash(password,12));});
mute=true;
